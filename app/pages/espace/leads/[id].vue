  ...
  <div class="max-w-2xl mx-auto px-6 py-16">
    ...
    <template v-else-if="lead">
      ...
      <div class="mb-8">
        ...
        <p class="text-xs text-muted-foreground mt-1">Reçu le {{ formattedDate }}</p>
      </div>
      ...
      <div class="border-t border-border pt-8 mb-8">
        ...
        <div class="border border-border rounded-lg divide-y divide-border">
          <div class="flex justify-between items-center px-5 py-3">
            ...
            <span class="text-sm font-semibold text-foreground">{{ lead.budget_range }}</span>
          </div>
          <div v-if="lead.timeline_range" class="flex justify-between items-center px-5 py-3">
            ...
            <span class="text-sm font-semibold text-foreground">{{ TIMELINE_LABELS[lead.timeline_range] ?? lead.timeline_range }}</span>
          </div>
          <div class="px-5 py-3">
            <span class="text-sm text-muted-foreground block mb-1">Description</span>
            <span v-if="isUnlocked" class="text-sm text-foreground">{{ lead.description }}</span>
            <span v-else class="text-sm text-muted-foreground font-mono select-none" aria-hidden="true">*** *** ***</span>
          </div>
        </div>
      </div>
      ...
      <div class="border-t border-border pt-8">
        ...
        <template v-else-if="isLocked">
          <div class="p-5 border border-amber-300 bg-amber-50 rounded-lg">
            ...
            <NuxtLink
              to="/espace/premium"
              class="inline-flex items-center gap-2 h-9 px-4 mt-4 bg-foreground text-background text-xs font-semibold rounded-md hover:opacity-80 transition-opacity"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <template v-else>
          <div class="border border-border rounded-lg p-5">
            <p class="text-sm text-muted-foreground text-center">Ce lead a déjà été attribué à un autre professionnel.</p>
          </div>
        </template>
      </div>
